    ...
    if (!isValid(temp)) {
      ...
      if (!isAlpha(properspacing)) {
        res
          .status(400)
          .send({
            err: "Invalid request parameters-> Please provide valid college abbreviation. The abbreviation name should only contains alphabets. ",
          });
      } else {
        ...
        if (!isValidRequestBody(requestBody)) {
          res
            ...
            .send({
              status: false,
              message:
                "Invalid request parameters. Please provide college details",
            });
          return;
        }
