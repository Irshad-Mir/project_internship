  ...
  try {
    ...
    if (!isValid(name)) {
      res
        ...
        .send({
          status: false,
          message: "Invalid request parameters. Please provide valid name",
        });
      return;
    }
    ...
    if (!isValid(mobile)) {
      res
        ...
        .send({
          status: false,
          message: "Invalid request parameters. Please provide valid mobile",
        });
      return;
    }
    ...
    if (!regexMobile.test(mobile)) {
      res
        .status(400)
        .send({ status: false, message: `number should be valid ` });
      return;
    }
    ...
    if (!isValid(collegeName)) {
      res
        ...
        .send({
          status: false,
          message:
            "Invalid request parameters. Please provide valid collegeName .",
        });
      return;
    }
    ...
    if (!collegeData) {
      res
        ...
        .send({
          status: false,
          message: `${nm} is not a valid college name or There might be spacing or letters are in captial`,
        });
    } else {
    ...
    if (!queryname) {
      ...
      if (propername != queryname) {
        res
          ...
          .send({
            status: false,
            err: "college abbreviation contains space , try writing without any space",
          });
      } else {
        ...
        if (tempcolgName != tempcolgName.toLowerCase()) {
          res
            ...
            .send({
              status: false,
              err: "college abbreviation should be in LowerCase / Small. Write in small letters and try again",
            });
        } else {
          ...
          if (!temp) {
            res
              ...
              .send({
                status: false,
                err: "Invalid parameters: Provide a valid college abbreviation",
              });
          } else {
            let ID = temp._id;
            let data = temp;
            ...
            let interns = await InternModel.find({ collegeId: ID }).select({
              _id: 1,
              name: 1,
              email: 1,
              mobile: 1,
            ...
            if (interns.length == 0) {
              ...
              res
                ...
                .send({
                  status: true,
                  data: details,
                  msg: "No Interns applied for an internship",
                });
            } else {
              let details = {
                name: data.name,
